...
- $nsteps \gets \left\lfloor \frac{b - a}{h} \right\rfloor$
...
The order of the methods $n$ can be approximated using

$$ n \approx \frac{\log(E_n(h_1) - \log(E_n(h_2))}{\log(h_1) - \log(h_2)}, $$

For example, for the RK4 method
...
$$ r = \max\left(0.1, 0.8 \left( \frac{tol}{\Delta} \right)^{\frac{1}{p+1}} \right). $$
...
- While $t_n < b$
    ...
    - If $\Delta < tol$
      ...
      - $t_{n+1} \gets t_n + h$
      - $n \gets n + 1$
    - $r \gets \max \left(0.1, 0.8 \left( \frac{tol}{\Delta} \right) ^{1/(p+1)} \right)$
...
### Python
```{.python code-line-numbers="0|1|4-5|13-18|20-21|22-25|27-28|30"}
def rk23(f, tspan, y0, atol=1e-6, rtol=1e-3):
...
```{.matlab  code-line-numbers="0|4-5|13-18|20-21|22-27|29-30|33-34"}
