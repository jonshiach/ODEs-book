...
# # The Taylor Series
# 
# The **Taylor series** is a series expansion from which the Runge-Kutta methods for solving initial value problems are derived. Given a differentiable function $f(t)$, the value $f(t+h)$ can be written using
# 
...
# ````
# 
# where $h$ is some (usually small) value known as the **step length**. We can use the Taylor series to approximate the value of $f(t)$ where the smaller the value of $h$, the closer the approximation $f(t+h)$ is to actual value $f(t)$. The problem with we have is that we cannot sum an infinite number of terms, so instead we only consider the first $n$ terms in the summation
# 
# $$f(t+h) = \underbrace{f(t) + hf'(t) + \cdots + \frac{h^n}{n!}f^{(n)}(t)}_{\mathrm{first }\,n\,\mathrm{ terms}} + \underbrace{\frac{h^{n+1}}{(n+1)!}f^{(n+1)}(t) + \frac{h^{n+2}}{(n+2)!}f^{(n+2)}(t) + \cdots}_{\mathrm{Highter\,Order\,Terms \,(H.O.T.)}}$$
# 
# It is import to note that since we are omitting the higher order terms in the truncation of the Taylor series we have an **approximation** of $f(t+h)$. When we omit the Higher-Order Terms (H.O.T.) from the Taylor series we say we have **truncated** the Taylor series. So if for example we omit all terms higher than fourth-order we have the *fourth-order Taylor series expansion* of $f(t+h)$.
# 
# $$f(t+h) = f(t) + hf' (t)+\frac{h^2}{2}f'' (t) + \frac{h^3}{3!}f^{''')}(t) + \frac{h^4}{4!}f^{(4)} + \mathrm{H.O.T.}$$
# 
...
# 
# The code below calculates the Taylor series approximation of $\cos(t + h)$ using the first $n$ terms of the series. Note that since the derivative of $\cos(t) = -\sin(t)$ then if $n$ is even then $f(t) = \cos$ and when $n$ is odd $f(t) = \sin(t)$. This is dealt with by using an `if` statement that checks whether the current value of $n$ is even or odd and calculates the value $f(t)$ accordingly (also the sign of the trigonometric function switches when $n$ is odd). 
