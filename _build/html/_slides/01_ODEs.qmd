...
```{.matlab code-line-numbers="0|1-3|4-7|9-10|12-16|18-22"}
% Define ODE function
f = @(t, y) t * y;
...
We saw that the local truncation error at each step is at most $\dfrac{Mh^2}{2}$ then after $n$ steps the upper bound of the global truncation error is at most $\dfrac{nMh^2}{2}$.
...
Since the error of the Euler method is $O(h)$, then it is a **first-order** method.
...
:::{.fragment}
The initial conditions are $\mathbf{y}_0 = (99, 1, 0)$ and $N = 100$, $\beta = 0.5$, $\gamma = 0.1$ so $\dfrac{\beta}{N} = 0.005$. 
...
```{.python code-line-numbers="0|6,9,12"}
def euler(f, tspan, y0, h):
...
```{.matlab code-line-numbers="0|6,9,12"}
function [t, y] = euler(f, tspan, y0, h)
...
```{.matlab code-line-numbers="0|1-12|16-18|22"}
% Define SIR function
...
$$ \begin{align*}
    \mathbf{y}_2 &= \mathbf{y}_1 + h f(t_1, \mathbf{y}_1), \\
    &= \begin{pmatrix} 1 \\ -0.4 \end{pmatrix} + 0.1 \begin{pmatrix} -0.4 \\ -2(-0.04) - 4(1) \end{pmatrix} 
