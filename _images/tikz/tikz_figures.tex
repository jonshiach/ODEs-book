...
\begin{tikzpicture}[scale=1.2] 
    \pgfmathdeclarefunction{f}{1}{\pgfmathparse{0.1*#1^2+1}}
    \pgfmathdeclarefunction{fd}{1}{\pgfmathparse{0.2*#1+0.925}}
    ...
    \draw[thick,blue,domain=0.5:5,smooth] plot (\x,{f(\x)}) node[right] {Exact solution}; 
    
    % Current point
    \coordinate (A) at (1.5,{f(1.5)}); \fill (A) circle (2pt); \node[below right] at (A) {$(t_0,y_0)$};
    
    % Tangent line 
    \draw[red,thick,domain=0.5:5,smooth] plot (\x,{fd(\x)}) node [right] {Tangent};
    
    % Step size h 
    \tikzmath{\h=2.5;}
    \draw[dashed] (1.5,0) -- ++(0,{f(1.5)}); 
    \draw[dashed] (1.5+\h,0) -- ++(0,{f(1.5+\h)}); 
    \draw[<->] (1.5,0.2) -- ++(\h,0) node [pos=0.5,above] {$h$}; 
    \node [below] at (1.5,0) {$t_0$};
    \node [below] at (1.5+\h,0) {$t_1$};
    
    % Euler approximation 
    \coordinate (B) at (1.5+\h,{fd(1.5+\h)}); 
    \fill (B) circle (2pt); 
    \node[below right] at (B) {$(t_{1},y_{1})$}; 
    
    % Exact value at t_{n+1} 
    \coordinate (C) at (1.5+\h,{f(1.5+\h)}); 
    \fill[blue] (C) circle (2pt); 
    \node[above left] at (C) {$y(t_1)$}; 
